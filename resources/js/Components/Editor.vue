...
export default {
    ...
    data() {
        return {
            ...
            editorConfig: {
                ...
                //         // { language: 'UIBlock', label: 'UI Block (Not Edit)', class: 'text-red-500' },
                ...
                // },
                htmlEmbed: {
                    showPreviews: true, // Hiển thị trước mục nhúng HTML
                    allowedProviders: ['*'], // Cho phép tất cả các nhà cung cấp
                },
                toolbar: {
                    items: [
    ...
    methods: {
                ...
                if (this.editorInstance) {
                    ...
                    this.editorInstance.model.change(writer => {
                        ...
                        data.forEach(file => {
                            ...
                            // Chèn hình ảnh vào vị trí con trỏ chuột hoặc vào cuối tài liệu nếu không có con trỏ chuột
                            if (selection.isCollapsed) {
                                this.editorInstance.model.insertContent(imageElement, selection);
                            } else {
                                const endPosition = this.editorInstance.model.createPositionAt(this.editorInstance.model.document.getRoot(), 'end');
                                writer.setSelection(endPosition);
                                this.editorInstance.model.insertContent(imageElement, endPosition);
                            }
                        });
                    });
                } else {
        ...
        insertUiBlock() {
            console.log(this.editer_data)
            if (this.editorInstance) {
                this.editorInstance.execute('htmlEmbed', '[banner]');
...
.ck-widget.raw-html-embed:before {
    content: 'VNWA Ui Block' !important;
}
